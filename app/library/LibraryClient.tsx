          ...
          <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
            <tbody>
              {rows.map((row: string[], i: number) => (
                <tr key={i} style={{ background: i === 0 ? '#f1f5f9' : i % 2 === 1 ? '#f9fafb' : '#fff' }}>
                  {row.map((cell: string, j: number) => i === 0
                    ? <th key={j} style={{ padding: '8px 12px', border: '1px solid #e2e8f0', fontWeight: 700, color: '#0f172a', textAlign: 'left', whiteSpace: 'nowrap' }}>{cell}</th>
                    : <td key={j} style={{ padding: '7px 12px', border: '1px solid #e2e8f0', color: '#374151' }}>{cell}</td>
                  )}
                </tr>
              ))}
            </tbody>
    ...
    if (content === '__binary__') {
      return (
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          ...
          <p style={{ color: '#64748b', fontSize: 13 }}>此文件类型不支持在线预览</p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ padding: '16px 28px 0', background: 'rgba(255,255,255,.75)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(59,130,246,.1)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              <h1 style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', margin: 0, letterSpacing: '-0.01em' }}>知识库</h1>
              <p style={{ fontSize: 12, color: '#64748b', margin: '3px 0 0' }}>管理企业知识文档{todayCount > 0 ? ` · 今日新增 ${todayCount} 份文件` : ''}</p>
            </div>
            {(canCreate || isAdmin) && (
                ...
                {isAdmin && selectedDs && (
                  <button onClick={handleRunGraphRAG} disabled={graphragRunning}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px',
                      background: graphragRunning ? '#f97316' : '#fff',
              ...
              <div key={ds.id} style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <button
                  ...
                  style={{
                    ...
                    color: selectedDs?.id === ds.id ? '#2563eb' : '#64748b',
                  ...
                  <span style={{ marginLeft: 5, fontSize: 12, opacity: 0.6,
                    ...
                    color: selectedDs?.id === ds.id ? '#2563eb' : '#94a3b8',
                ...
                {isAdmin && (
                  <button onClick={() => handleDeleteDataset(ds)} title="删除此知识库"
                    style={{ position: 'absolute', top: 2, right: -2, width: 14, height: 14, borderRadius: '50%', background: '#ef4444', border: 'none', color: '#fff', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.15s' }}
                    onMouseEnter={e => (e.currentTarget as HTMLElement).style.opacity = '1'}
                    onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = '0'}>×</button>
                )}
              </div>
            ))}
            {datasets.length === 0 && <span style={{ fontSize: 12, color: '#94a3b8', paddingBottom: 10 }}>暂无知识库</span>}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 28px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {canUpload && selectedDs && (
            <div
              ...
              style={{ border: `1.5px dashed ${drag ? '#3b82f6' : '#c7d8f8'}`, borderRadius: 12, padding: '20px',
              ...
              <svg style={{ width: 22, height: 22, color: drag ? '#3b82f6' : '#3b82f6', marginBottom: 2 }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>
              <p style={{ fontSize: 13, fontWeight: 500, color: drag ? '#3b82f6' : '#475569', margin: 0 }}>
              ...
              <p style={{ fontSize: 12, color: '#94a3b8', margin: 0 }}>PDF · Word · Excel · PPT · TXT · Markdown</p>
            </div>
          )}
          ...
          {/* Unified filter bar */}
          {selectedDs && (
            <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', flexShrink: 0 }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <svg style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', width: 13, height: 13, color: '#94a3b8', pointerEvents: 'none' }} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z" /></svg>
                <input value={filterSearch} onChange={e => setFilterSearch(e.target.value)} placeholder="搜索文件名..."
                  style={{ paddingLeft: 28, paddingRight: 10, paddingTop: 6, paddingBottom: 6, borderRadius: 7, border: `1.5px solid ${filterSearch ? '#3b82f6' : '#e2e8f0'}`, fontSize: 13, color: '#374151', outline: 'none', width: 160, background: filterSearch ? '#eff6ff' : '#f8fafc', transition: 'border-color 0.15s' }} />
              </div>
              <div style={{ width: 1, height: 20, background: '#e2e8f0', flexShrink: 0 }} />
              {uniqueUploaders.length > 0 && (
                <select value={filterUploader} onChange={e => setFilterUploader(e.target.value)}
                  style={{ padding: '6px 10px', borderRadius: 7, border: `1.5px solid ${filterUploader ? '#3b82f6' : '#e2e8f0'}`, fontSize: 12, background: filterUploader ? '#eff6ff' : '#f8fafc', color: filterUploader ? '#2563eb' : '#64748b', outline: 'none', cursor: 'pointer' }}>
                  <option value=''>全部上传者</option>
                  {uniqueUploaders.map(u => <option key={u} value={u}>{u}</option>)}
                </select>
              )}
              <select value={filterDept} onChange={e => setFilterDept(e.target.value)}
                style={{ padding: '6px 10px', borderRadius: 7, border: `1.5px solid ${filterDept ? '#3b82f6' : '#e2e8f0'}`, fontSize: 12, background: filterDept ? '#eff6ff' : '#f8fafc', color: filterDept ? '#2563eb' : '#64748b', outline: 'none', cursor: 'pointer' }}>
                <option value=''>全部部门</option>
                {DEPTS.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              <select value={filterType} onChange={e => setFilterType(e.target.value)}
                style={{ padding: '6px 10px', borderRadius: 7, border: `1.5px solid ${filterType ? '#3b82f6' : '#e2e8f0'}`, fontSize: 12, background: filterType ? '#eff6ff' : '#f8fafc', color: filterType ? '#2563eb' : '#64748b', outline: 'none', cursor: 'pointer' }}>
                <option value=''>全部类型</option>
                {DOC_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <div style={{ width: 1, height: 20, background: '#e2e8f0', flexShrink: 0 }} />
              <select value={sortBy} onChange={e => setSortBy(e.target.value as 'date'|'name'|'size')}
                style={{ padding: '6px 10px', borderRadius: 7, border: '1.5px solid #e2e8f0', fontSize: 12, background: '#f8fafc', color: '#64748b', outline: 'none', cursor: 'pointer' }}>
          ...
          {/* File list */}
          {!selectedDs ? (
            <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8', fontSize: 13 }}>{canCreate ? '请先创建或选择一个知识库' : '暂无可用知识库，请联系管理员'}</div>
          ) : (
            <div style={{ background: '#fff', borderRadius: 14, border: '1px solid rgba(59,130,246,.1)', boxShadow: '0 2px 12px rgba(59,130,246,.06)', overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, opacity: loadingDocs ? 0.45 : 1, transition: 'opacity 0.18s', pointerEvents: loadingDocs ? 'none' : 'auto' }}>
              {/* List header */}
              <div style={{ padding: '10px 18px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', flexShrink: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                  {currentFolderId ? (
                    ...
                    <span style={{ fontWeight: 700, fontSize: 14, color: '#0f172a' }}>{selectedDs.name}</span>
                  )}
          ...
          {/* Stats cards */}
          {selectedDs && (
            <div style={{ flexShrink: 0, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, paddingBottom: 4 }}>
              {[
                { label: '全部文档', value: docs.length, trend: `今日 +${todayCount}`, gradFrom: '#3b82f6', gradTo: '#6366f1', trendColor: '#10b981' },
                { label: '已解析完成', value: docs.filter(d => d.run === 'DONE').length, trend: docs.length ? Math.round(docs.filter(d => d.run === 'DONE').length / docs.length * 100) + '% 完成率' : '—', gradFrom: '#059669', gradTo: '#10b981', trendColor: '#059669' },
                { label: '待处理', value: docs.filter(d => ['RUNNING','PENDING','UNSTART'].includes(d.run)).length, trend: '解析队列中', gradFrom: '#d97706', gradTo: '#f59e0b', trendColor: '#d97706' },
                { label: '文件夹', value: folders.length, trend: '已分类归档', gradFrom: '#0284c7', gradTo: '#38bdf8', trendColor: '#0284c7' },
              ].map(({ label, value, trend, gradFrom, gradTo, trendColor }) => (
                <div key={label} style={{ background: '#fff', border: '1px solid rgba(59,130,246,.1)', borderTop: `2px solid rgba(59,130,246,.2)`, borderRadius: 14, padding: '14px 16px', boxShadow: '0 2px 12px rgba(59,130,246,.06)', transition: 'all .2s', cursor: 'default' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 24px rgba(59,130,246,.12)' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 2px 12px rgba(59,130,246,.06)' }}>
                  <div style={{ fontSize: 24, fontWeight: 700, background: `linear-gradient(135deg,${gradFrom},${gradTo})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{value}</div>
      ...
      {pendingFiles && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', margin: '0 0 6px' }}>上传文件信息</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <select value={uploadDept} onChange={e => setUploadDept(e.target.value)}
                  style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #e2e8f0', fontSize: 13, color: '#0f172a', outline: 'none', background: '#fff', boxSizing: 'border-box' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', userSelect: 'none', padding: '10px 14px', background: uploadConf ? '#fff5f5' : '#f8fafc', border: `1.5px solid ${uploadConf ? '#fca5a5' : '#e2e8f0'}`, borderRadius: 8 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#94a3b8' }}>机密文件仅中级及以上权限用户可下载</div>
                </div>
              </label>
            </div>
            <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
              <button onClick={() => { setPendingFiles(null); if (fileRef.current) fileRef.current.value = '' }}
                style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1.5px solid #e2e8f0', background: '#fff', color: '#374151', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>取消</button>
              <button onClick={confirmUpload} disabled={uploading}
                style={{ flex: 1, padding: '10px', borderRadius: 8, border: 'none', background: uploading ? '#93c5fd' : '#2563eb', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                {uploading ? '上传中...' : '确认上传'}
              </button>
            </div>
          </div>
            ...
            <div style={{ padding: '18px 24px 14px', borderBottom: '1px solid #e2e8f0', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                  {previewDoc.meta?.dept && <span style={{ fontSize: 12, padding: '1px 7px', borderRadius: 999, background: '#dbeafe', color: '#1d4ed8', fontWeight: 500 }}>{previewDoc.meta.dept}</span>}
                  {previewDoc.meta?.type && <span style={{ fontSize: 12, padding: '1px 7px', borderRadius: 999, background: '#f3e8ff', color: '#7c3aed', fontWeight: 500 }}>{previewDoc.meta.type}</span>}
                  {previewDoc.meta?.confidential && <span style={{ fontSize: 12, padding: '1px 7px', borderRadius: 999, background: '#fee2e2', color: '#dc2626', fontWeight: 500 }}>机密</span>}
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>{fmtSize(previewDoc.size)}</span>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => { setPreviewDoc(null); setPreviewContent(''); setPreviewLoading(false) }}
                  style={{ background: 'none', border: 'none', fontSize: 18, color: '#94a3b8', cursor: 'pointer', padding: '4px 8px', borderRadius: 6 }}>×</button>
              </div>
            </div>
            {previewDoc.meta?.ai_summary && (
              <div style={{ padding: '10px 24px', background: '#f0fdf4', borderBottom: '1px solid #bbf7d0', flexShrink: 0 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#16a34a', marginRight: 6 }}>AI摘要</span>
      ...
      {showTrash && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 600, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ padding: '18px 24px 14px', borderBottom: '1px solid #e2e8f0', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <h2 style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', margin: 0 }}>回收站</h2>
                <p style={{ fontSize: 12, color: '#64748b', margin: '3px 0 0' }}>已软删除的文件，可恢复或永久删除</p>
              </div>
              <button onClick={() => setShowTrash(false)} style={{ background: 'none', border: 'none', fontSize: 18, color: '#94a3b8', cursor: 'pointer' }}>×</button>
            </div>
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 420, boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: '#0f172a', margin: '0 0 20px' }}>新建知识库</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
              ...
              <div>
                ...
                <textarea value={newDsDesc} onChange={e => setNewDsDesc(e.target.value)} rows={2}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1.5px solid #e2e8f0', fontSize: 13, outline: 'none', resize: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
              <button onClick={() => { setShowCreate(false); setNewDsName(''); setNewDsDesc('') }} style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1.5px solid #e2e8f0', background: '#fff', color: '#374151', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>取消</button>
              <button onClick={createDataset} disabled={creating || !newDsName.trim()} style={{ flex: 1, padding: '10px', borderRadius: 8, border: 'none', background: creating || !newDsName.trim() ? '#93c5fd' : '#2563eb', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                {creating ? '创建中...' : '创建'}
              </button>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 16 }}>
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 380, boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
              <button onClick={() => setShowCreateFolder(false)} style={{ flex: 1, padding: '10px', borderRadius: 8, border: '1.5px solid #e2e8f0', background: '#fff', color: '#374151', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>取消</button>
              <button onClick={createFolder} disabled={creatingFolder || !newFolderName.trim()} style={{ flex: 1, padding: '10px', borderRadius: 8, border: 'none', background: creatingFolder || !newFolderName.trim() ? '#fde68a' : '#d97706', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 460, boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <input value={audioTitle} onChange={e => setAudioTitle(e.target.value)} placeholder="留空则使用文件名" style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={() => setShowAudio(false)} style={{ flex: 1, padding: '9px', borderRadius: 8, border: '1.5px solid #e2e8f0', background: '#fff', color: '#374151', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>关闭</button>
              <button onClick={handleAudioSubmit} disabled={audioLoading || !audioFile} style={{ flex: 1, padding: '9px', borderRadius: 8, border: 'none', background: audioLoading || !audioFile ? '#c4b5fd' : '#7c3aed', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 28, width: '100%', maxWidth: 460, boxShadow: '0 20px 50px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <input value={wechatUrl} onChange={e => setWechatUrl(e.target.value)} placeholder="https://mp.weixin.qq.com/s/..." autoFocus style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: '#374151', marginBottom: 5 }}>自定义标题（可选）</label>
                <input value={wechatTitle} onChange={e => setWechatTitle(e.target.value)} placeholder="留空则使用原标题" style={{ width: '100%', padding: '9px 12px', borderRadius: 8, border: '1.5px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            </div>
            {wechatResult && (
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={() => setShowWechat(false)} style={{ flex: 1, padding: '9px', borderRadius: 8, border: '1.5px solid #e2e8f0', background: '#fff', color: '#374151', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>关闭</button>
              <button onClick={handleWechatSubmit} disabled={wechatLoading || !wechatUrl.trim()} style={{ flex: 1, padding: '9px', borderRadius: 8, border: 'none', background: wechatLoading || !wechatUrl.trim() ? '#86efac' : '#16a34a', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
