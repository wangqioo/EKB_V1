...
export default function AdminPage() {
  ...
  if (loading) return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-tertiary)', fontSize: 14 }}>加载中...</div>
  )
  ...
  return (
      <div style={{ flex: 1, overflow: 'auto', padding: '32px 40px', position: 'relative', zIndex: 1 }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>管理企业用户账号与权限</p>
        </div>

        {/* Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(140px, 200px))', gap: 14, marginBottom: 28 }}>
          {[
            { label: '全部用户', value: users.length, color: 'var(--accent)', bg: '#eff6ff' },
            { label: '管理员', value: adminCount, color: '#f59e0b', bg: '#fffbeb' },
            { label: '普通员工', value: users.length - adminCount, color: '#22c55e', bg: '#f0fdf4' },
          ].map(s => (
            <div key={s.label} style={{ background: 'var(--bg-surface)', borderRadius: 14, border: '1px solid var(--border)', padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <div style={{ background: 'var(--bg-surface)', borderRadius: 14, border: '1px solid var(--border)', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
            <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)' }}>用户列表</span>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border)' }}>
                {['用户', '角色', '权限等级', ''].map(h => (
                  <th key={h} style={{ padding: '11px 20px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={u.id} style={{ borderBottom: i < users.length - 1 ? '1px solid #f8fafc' : 'none', opacity: saving === u.id ? 0.6 : 1 }}
                  ...
                  <td style={{ padding: '13px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: '50%', background: u.role === 'admin' ? '#fef3c7' : '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, color: u.role === 'admin' ? '#d97706' : 'var(--accent)', flexShrink: 0 }}>
                        {u.name.charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{u.name}</div>
                        <div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>@{u.id}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '13px 20px' }}>
                    <select value={u.role} onChange={e => update(u.id, 'role', e.target.value)} disabled={saving === u.id}
                      style={{ fontSize: 13, padding: '5px 10px', borderRadius: 8, border: '1px solid var(--border-strong)', color: 'var(--text-primary)', background: 'var(--bg-surface)', cursor: 'pointer', outline: 'none' }}>
                  ...
                  <td style={{ padding: '13px 20px', fontSize: 12, color: 'var(--text-tertiary)' }}>
                    {saving === u.id ? '保存中...' : ''}
                  </td>
                </tr>
              ))}
        ...
        <div style={{ marginTop: 20, background: 'var(--bg-surface)', borderRadius: 14, border: '1px solid var(--border)', padding: '16px 20px', fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.8 }}>
          <strong style={{ color: 'var(--text-primary)' }}>权限说明：</strong>
          基础权限 — 仅可使用 AI 问答；
          中级权限 — 可上传文档 + 问答；
          最高权限 — 可新建知识库 + 上传 + 问答；
