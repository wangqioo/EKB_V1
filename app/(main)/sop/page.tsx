      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '40px 48px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 800 }}>
          <div style={{ marginBottom: 32 }}>
            <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)', margin: '0 0 8px' }}>企业知识库平台 — 使用指南</h1>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', margin: 0 }}>SOP 流程化展示 · 帮助您快速上手平台各功能</p>
          </div>

          {[
            {
              step: '01', title: '登录与权限说明', color: 'var(--accent)', bg: '#dbeafe',
              items: [
            ...
            <div key={section.step} style={{ background: 'var(--bg-surface)', borderRadius: 16, border: '1px solid var(--border)', padding: '24px', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
                <div style={{ width: 40, height: 40, borderRadius: 14, background: section.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: section.color }}>{section.step}</span>
                </div>
                <h2 style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>{section.title}</h2>
              </div>
              <ul style={{ margin: 0, paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {section.items.map((item, i) => (
                  <li key={i} style={{ fontSize: 14, color: 'var(--text-primary)', lineHeight: 1.7 }}>{item}</li>
                ))}
              </ul>
            </div>
          ))}

          <div style={{ background: 'var(--text-primary)', borderRadius: 16, padding: '24px', color: 'var(--text-tertiary)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--bg-elevated)', margin: '0 0 12px' }}>常见问题</h3>
            {[
              ...
              <div key={i} style={{ marginBottom: 14 }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7 }}>A: {a}</div>
              </div>
            ))}
          </div>
        </div>
